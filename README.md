...
### 2주차 (W2) — 방향 전환(세그 → 합성/콜라주) + “얼굴 품질” 문제 정의
- “세그먼트 기반 파이프라인”에서 **이미지 합성/콜라주 기반 접근**으로 방향을 조정
- 생성 이미지에서 특히 **얼굴이 어색하게 나오는 문제**를 핵심 리스크로 정의
- 해결 후보로 **Face Enhancement / Face Swap(원본 얼굴 이식)** 파이프라인을 리서치·테스트  
  - 예: YOLO Face → CodeFormer(복원) → Face Swap(이식) 조합 등
- 멘토링 피드백을 바탕으로 “생성 품질(얼굴/인물 동일성) 확보”를 이후 주차의 우선순위로 확정

### 3주차 (W3) — 1차 데모 실험: 키포인트/생성모델 조합 탐색
...
- “실험 로그/회의록/디버깅 테이블”을 기반으로, 매주 의사결정과 기술 변경(예: 세그→합성/콜라주, OpenPose→DW Pose, 1-stage→2-stage)을 빠르게 반복했습니다.
## 기획 의도
